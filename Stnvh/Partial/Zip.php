...
class Zip {
	 ...
	 * @return void
	 */
	public function __construct($url = false, $local_file_path = false, $file = false) {
		$this->info = new Data\ZipInfo();
		if($url){
			$this->info->url = $url;
		}else if($local_file_path){
			$this->info->local_file = $local_file_path;
		}else{
			throw new RuntimeException('must provide url or local_file_path');
		}
	...
	public function init() {
		...
		if($this->info->url){
			...
			$this->info->length = filesize($this->info->local_file);
		}
	...
	private function getRange($start, $end) {
		if($this->info->url){
			...
			// echo 'file-range:'.$start.'-'.$end.PHP_EOL;
			$contents = file_get_contents($this->info->local_file, 1, NULL, $start,  $end-$start+1);
			return $contents;
		}
	}
